...
const WEEK_ONE_FRIDAY = '2026-07-17';
const WEEK_TWO_FRIDAY = '2026-07-24';
...
export async function runAdditionalChefTests(assert) {
  ...
  assert(validateAdditionalChefDate(WEEK_ONE_FRIDAY, WEEK_START) === '', 'Additional-chef date inside single-week range is accepted');
  assert(validateAdditionalChefDateForHorizon(WEEK_TWO_FRIDAY, WEEK_START, 2) === '', 'Additional-chef date inside later selected week is accepted');
  ...
  assert(state.weeklyInputs.additionalChefRequirements[0].date === WEEK_TWO_FRIDAY, 'Editing an additional-chef request can move it to a later week');
  removeAdditionalChefRequest(WEEK_TWO_FRIDAY);
  assert(state.weeklyInputs.additionalChefRequirements.length === 0, 'Removing an additional-chef request clears it');

  state.weeklyInputs.additionalChefRequirements = [{ date: WEEK_TWO_FRIDAY, count: 2 }];
  ...
  const weekTwoFriday = multiWeek.weeks[1].rota.find((day) => day.date === WEEK_TWO_FRIDAY);
  assert(new Set(weekTwoFriday.chefs).size === 7, 'Additional-chef requests work in later weeks');

  state.weeklyInputs.additionalChefRequirements = [{ date: WEEK_ONE_FRIDAY, count: 1 }];
  ...
  const friday = singleWeek.rota.find((day) => day.date === WEEK_ONE_FRIDAY);
